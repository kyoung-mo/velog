<h3 id="정의">정의</h3>
<p>데이터를 여러 개의 버킷에 나누고, 각 버킷을 개별적으로 정렬한 다음 결과를 합치는 방식입니다.</p>
<h3 id="장점-및-단점">장점 및 단점</h3>
<p>주로 데이터가 균등하게 분포해 있을 때 효율적입니다.</p>
<p>그러나 데이터가 한쪽으로 쏠려 있으면 정렬 성능이 저하됩니다.</p>
<h3 id="과정">과정</h3>
<p>버킷 정렬은 데이터 분할, 정렬, 합치기 과정을 수행합니다.</p>
<p>“분할” 과정에서는 비슷한 값끼리 한 버킷에 모으고</p>
<p>“정렬” 과정에서는 각 버킷을 정렬한 뒤에</p>
<p>“합치기” 과정에서 각 버킷을 순서대로 이어붙입니다.</p>
<h3 id="상세-과정">상세 과정</h3>
<p>분할</p>
...
<p>버킷을 10개로 나누어 버킷[0] 에 한자리 숫자인 7을, 버킷[1] 은 비워져 있으므로 건너뛰고, 버킷[2] 에는 십의자리 숫자가 2인 항목을 넣습니다. 계속해서 버킷[9] 까지 채워나갑니다.</p>
<p><img alt="" src="https://velog.velcdn.com/images/mommers/post/dd9a59c2-48d9-4e37-8c0f-746d17e2478d/image.png" /></p>
<p>버킷[0]과 버킷[1] 자리는 비어있거나 값이 하나이므로 건너뜁니다.</p>
<p>버킷[2] 자리에는 index 0~2 이 존재하므로 index 1(그림상 25) 과 index 0, index 2 각각 비교하여 낮은숫자가 낮은 index 가 되도록 삽입 정렬합니다.</p>
<p><img alt="" src="https://velog.velcdn.com/images/mommers/post/5e94aab5-5d08-43fc-b8f8-cd34142a9b3c/image.png" /></p>
<p>버킷[3] 자리에는 index 0~1 이 존재하므로 index 1(그림상 31)과 index 0 을 비교하여 낮은숫자가 낮은 index 가 되도록 삽입 정렬합니다.</p>
...
<h3 id="예시-코드-및-결과"><strong>예시 코드 및 결과</strong></h3>
...
<pre><code class="language-jsx">bucket[0] : 1224 36570 95929 78287 26908
